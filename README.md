...
1. Using Git, clone the https://github.com/cloudbees-io/springboot-fm-example repository to your local machine:
...
6. Run the application in your IDE. For example:

     ```shell
     ./gradlew bootRun
     ```
     
7. To view the application running, go to `http://localhost:8080/api/demo` in your web browser.
...
| `fontSize`    | Number    | Sets the message font size in pixels. Values: `12`, `16`, or `24`. |
| `fontColor`   | String    | Sets the message font color. Values: `red`, `green`, or `blue`. |

> **Note:**  
> If no flags are displayed, verify that the environment-specific SDK key is correctly added to:
> `src/main/resources/application.yaml`  
