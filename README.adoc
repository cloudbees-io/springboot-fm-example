...
.. Select image:ROOT:icon-copy-dark.png[COPY] next to the SDK key on the page.
+
[NOTE]
====
...
. Navigate to menu:Feature management[Flags].
. Select image:ROOT:installinstructions.png[install,100] in the upper-right corner.
...
. Select image:ROOT:icon-vertical-ellipsis-dark.png[Vertical ellipsis] next to the flag you want to configure.
. Select *Configure*.
...
For more information about using multiple SDK keys, refer to xref:cloudbees-platform:feature-management:use-multiple-sdk-keys.adoc[].

For installation details, refer to xref:cloudbees-platform:install-sdk:index.adoc[].
